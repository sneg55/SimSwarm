...
describe('WizardLaunch', () => {
  ...
  it('disables unaffordable tiers', () => {
    const store = useCreditsStore()
    store.balance = 50  // can afford small (30) but not medium (90) or large (300)
    const wrapper = mount(WizardLaunch, {
      props: {},
      global: { stubs: { RouterLink: RouterLinkStub } },
    })
  ...
  it('marks large as needed for long forecastDays', () => {
    const store = useCreditsStore()
    store.balance = 500
    const wrapper = mount(WizardLaunch, {
      props: { forecastDays: 365 },
      global: { stubs: { RouterLink: RouterLinkStub } },
    })
  ...
  it('selecting a tier emits update:tier', async () => {
    ...
    // initial emit (medium) + possible small
