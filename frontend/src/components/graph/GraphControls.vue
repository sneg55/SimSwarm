<template>
  <div class="flex items-center gap-1.5">
    ...
    <button
      @click="$emit('refresh')"
      class="p-1.5 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-50 transition-colors"
      title="Re-layout"
    >
    ...
    >
      <svg v-if="!isFullscreen" class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <!-- Edge Labels toggle -->
    <div
      class="flex items-center gap-1.5 px-2 py-1 bg-white border border-gray-200 rounded-lg shadow-sm cursor-pointer select-none"
    ...
    <select
      ...
      class="text-xs bg-white border border-gray-200 rounded-lg shadow-sm px-2 py-1.5 text-gray-600 focus:outline-none focus:ring-1 focus:ring-indigo-400"
    >
    ...
    <div class="relative" ref="exportDropdownRef">
      ...
      <div
        v-if="exportOpen"
        class="absolute right-0 mt-1 w-24 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden z-10"
      >
        <button
