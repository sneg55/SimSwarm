...
describe('api/config', () => {
  it('getConfig GETs /config and returns data', async () => {
    api.get.mockResolvedValue({ data: { demo_mode: true } })
    const result = await config.getConfig()
    expect(api.get).toHaveBeenCalledWith('/config')
    expect(result).toEqual({ demo_mode: true })
  })

  it('getConfig propagates API errors', async () => {
    api.get.mockRejectedValue(new Error('Network'))
    await expect(config.getConfig()).rejects.toThrow('Network')
  })
})
