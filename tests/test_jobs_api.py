...
async def test_create_job(client, auth_headers, funded_user):
...
async def test_create_job_seed_too_long(client, auth_headers, funded_user):
    response = await client.post(
        "/api/jobs",
        headers=auth_headers,
        json={
...
async def test_get_job_status(client, auth_headers, funded_user):
...
async def test_list_user_jobs(client, auth_headers, funded_user):
    for _ in range(2):
        await client.post(
            "/api/jobs",
            headers=auth_headers,
