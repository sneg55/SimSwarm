...
def _stale_job(age_seconds: int, status: str = "PROVISIONING", workflow_id: str = "sim-1"):
    return {
        ...
        "credits_charged": 30, "pod_id": "pod-xyz",
...
@patch("saas.jobs.refund._refund_credits")
@patch("saas.jobs.persistence._mark_job_failed_sync")
def test_reconcile_refunds_and_terminates_pod(
    mock_mark_failed, mock_refund, mock_terminate,
):
    ...
    mock_refund.assert_called_once_with(job_id=1, user_id="u1", credits=30)
...
@patch("saas.jobs.refund._refund_credits")
@patch("saas.jobs.persistence._mark_job_failed_sync")
def test_reconcile_skips_refund_when_no_credits_charged(
    mock_mark_failed, mock_refund, mock_terminate,
):
    from saas.jobs.stale_detector import _reconcile_stale_job

    job = _stale_job(age_seconds=9999)
