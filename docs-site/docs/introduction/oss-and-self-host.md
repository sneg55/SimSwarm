...
SimSwarm is MIT-licensed and self-hostable. The whole stack ships in one repository: the
native simulation engine, the FastAPI backend, the Vue frontend, and the GPU job runner.
There is no commercial billing layer. Authenticated users on a self-hosted instance
submit simulations freely.
...
external services the pipeline depends on: a GPU provider for the simulation workers, an
LLM provider for report generation, and object storage for simulation artifacts. The
exact set of environment variables and the bring-up steps are covered in the
