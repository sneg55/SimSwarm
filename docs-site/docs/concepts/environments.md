...
When an environment's tools need an identifier, that identifier is
...
instead of only talking about it. Rather than starting empty, its markets are derived
from the goal before the run begins: an LLM call turns the goal into a small set of
binary (YES/NO) prediction markets with clear resolution criteria, and those seed the
