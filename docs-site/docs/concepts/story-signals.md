...
human-readable picture of what happened — who took which side, how the run unfolded over
...
Story signals are computed by pure functions — no LLM calls and no external I/O — so
...
- **Quotable posts** — the highest-engagement post per phase per stance, deduplicated so
  the same agent isn't quoted twice — the lines that carried the conversation.
- **Simulation scale** — honest aggregates about the run: how many participants took
...
much weight to give the result — for instance, whether market stress was genuinely
