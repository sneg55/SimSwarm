...
- **app (FastAPI)** is the API surface. The router (`saas/router.py`) mounts health, jobs, auth, progress, export, share, fetch, profile, and AI routes under `/api`. It reads/writes application state in PostgreSQL, and on job creation it generates presigned MinIO URLs and starts a Temporal workflow.
...
- **temporal** + **temporal-db** hold authoritative workflow state. The app is a Temporal client; it does not store workflow progress itself.
