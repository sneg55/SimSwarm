...
- During provisioning, the poll loop is wrapped so that cancellation (e.g. a Temporal activity timeout) terminates the pod before re-raising, so a cancelled provision never leaks a billing pod.
