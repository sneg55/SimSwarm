...
For each stale job we mark it FAILED and (best-effort) terminate any
lingering pod. This closes the gap left by F1 (upload stall), F2 (celery
report worker down), and T1 (temporal data loss).
...
def _load_live_jobs() -> list[dict]:
    ...
    try:
        with engine.connect() as conn:
            rows = conn.execute(
                text(
                    "SELECT id, user_id, status, tier, pod_id, "
...
def _reconcile_stale_job(job: dict, reason: str) -> None:
    """Mark the job FAILED and terminate its pod."""
    ...
    logger.warning(
        "stale.reconciled job_id=%d status_was=%s reason=%s",
        job_id, job["status"], reason,
    )
