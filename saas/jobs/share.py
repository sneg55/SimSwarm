...
async def get_shared_result(
    ...
    # Adapt to the Graphiti shape the frontend graph components expect
    # (uuid/name on nodes; source_node_uuid/_name, name, fact on edges) —
    # same adaptation the /share/{token}/graph endpoint applies. Without
    # this, shared/demo graphs render nodes but no edges/relationships.
    graph_data = adapt_graph_payload(json.loads(job.result_graph)) if job.result_graph else None
