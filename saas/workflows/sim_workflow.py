...
retry policies, wires them together, and marks the job FAILED on any failure
(pre-GPU or in the GPU-phase try block). GPU teardown runs in the finally.
"""
...
class SimulationWorkflow:
    ...
    async def run(self, params: SimParams) -> None:
        ...
        # pod exists yet, so a failure here bypasses the Phase 2 handler — we
        # mark the job FAILED explicitly before the workflow raises.
        #
        ...
        try:
            ...
            await workflow.execute_activity(
                "fishcloud.mark_failed",
                args=[params.job_id, f"pre_gpu_failed: {str(e)[:4000]}"],
        ...
        try:
            ...
            # that mark_failed fires, the pod is torn down, and the stale-job
            ...
            # Mark the job FAILED before propagating; teardown runs in finally.
            await workflow.execute_activity(
                "fishcloud.mark_failed",
                args=[params.job_id, str(e)[:4096]],
                start_to_close_timeout=timedelta(seconds=30),
                retry_policy=RetryPolicy(maximum_attempts=5),
            )
            raise
